...
void view::MiniMap::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    ...
    // Outline
    {
        ...
        QPen pen(bgColor, 1.0 * _invScale);

        QBrush backgroundBrush(bgColor);
        backgroundPath.addRect(0, 0, rect.width(), rect.height());
        painter->setPen(pen);
    ...
    // Viewport area
    {
        ...
        QBrush backgroundBrush(bgColor);
        backgroundPath.addRect(x, y, w, h);
        painter->setPen(pen);
...
void view::MiniMap::mousePressEvent(QGraphicsSceneMouseEvent* event)
{
    if (event->button() == Qt::MouseButton::LeftButton) {
        ...
        if (partialX < 0.0f || partialX > 1.0f)
            return;
        if (partialY < 0.0f || partialY > 1.0f)
            return;

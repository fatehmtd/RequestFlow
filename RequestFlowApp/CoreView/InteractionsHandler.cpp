...
view::InteractionsHandler::InteractionsHandler(SceneGraph* scene)
    : QObject(scene)
    , _sceneGraph(scene)
{
...
{
    ItemAction action(order, icon, name, func, filter);
...
{
    registerGenericAction(
        name, [nodeType](QGraphicsItem* item) {
...
{
    registerEmptySpaceAction(
        "Stop", [=](const QPointF& p, QAction* action) {
            _sceneGraph->getModelGraph()->cancel();
        },
        QIcon(":/BleachUI/stop"));

    registerEmptySpaceAction(
        "Execute", [=](const QPointF& p, QAction* action) {
    ...
    registerEmptySpaceAction(
        "New Payload", [=](const QPointF& p, QAction* action) {
    ...
    registerEmptySpaceAction(
        "New Endpoint", [=](const QPointF& p, QAction* action) {
    ...
    registerEmptySpaceAction(
        "New Viewer", [=](const QPointF& p, QAction* action) {
    ...
    registerEmptySpaceAction(
        "New Delay", [=](const QPointF& p, QAction* action) {
    ...
    registerEmptySpaceAction(
        "New Script", [=](const QPointF& p, QAction* action) {
    ...
    registerEmptySpaceAction(
        "New Assertion", [=](const QPointF& p, QAction* action) {
    ...
    registerEmptySpaceAction(
        "New External", [=](const QPointF& p, QAction* action) {
    ...
    registerNodeAction(
        "Duplicate", [=](const QPointF& p, QAction* action) {
            auto item = _sceneGraph->itemAt(p, QTransform());
    ...
    registerGenericAction(
        ...
        [](QGraphicsItem* item) {
            ...
            if (slot != nullptr) {
                if (slot->getModelSlot()->getDirection() != model::Slot::Direction::INPUT)
                    return false;

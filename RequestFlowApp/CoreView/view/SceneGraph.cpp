...
QList<view::Node *> view::SceneGraph::getNodeChildren(Node *node) const {
    ...
    auto modelChildrenNodes =
        getModelGraph()->getNodeChildren(node->getModelNode());
    std::for_each(
        modelChildrenNodes.begin(), modelChildrenNodes.end(),
        [this, &output](model::Node *child) { output << findbyModel(child); });
    return output;
}
...
QList<view::Edge *> view::SceneGraph::getEdges() const {
    ...
    std::for_each(modelEdgesList.begin(), modelEdgesList.end(),
                  [this, &edgesList](model::Edge *modelEdge) {
...
model::Graph *view::SceneGraph::getModelGraph() const { return _modelGraph; }
...
void view::SceneGraph::drawDotsBackground(QPainter *painter,
    ...
    int count = (1 + (right - firstLeft) / _cellSize) *
                (1 + (bottom - firstTop) / _cellSize);
...
void view::SceneGraph::drawGridBackground(QPainter *painter,
                                          const QRectF &rect) const {
    const int left = floor(rect.left());
    const int right = ceil(rect.right());
    const int top = floor(rect.top());
...
void view::SceneGraph::drawCrossBackground(QPainter *painter,
                                           const QRectF &rect) const {
    QColor lightGridColor = qApp->palette().color(QPalette::Highlight).lighter();
    // QColor darkGridColor = qApp->palette().color(QPalette::Highlight).darker();
...
void view::SceneGraph::drawQuadsBackground(QPainter *painter,
                                           const QRectF &rect) const {
    const int left = floor(rect.left());
    const int right = ceil(rect.right());
    const int top = floor(rect.top());
...
void view::SceneGraph::createEdge() {
    if (_connectionEdge->getHeadSlot() && _connectionEdge->getTailSlot()) {
        ...
        if (_connectionEdge->getDirection() ==
            ConnectionEdge::DESTINATION_TO_ORIGIN) {
            outputSlot = _connectionEdge->getTailSlot()->getModelSlot();
            inputSlot = _connectionEdge->getHeadSlot()->getModelSlot();
        } else if (_connectionEdge->getDirection() ==
                   ConnectionEdge::ORIGIN_TO_DESTINATION) {
            inputSlot = _connectionEdge->getTailSlot()->getModelSlot();
            outputSlot = _connectionEdge->getHeadSlot()->getModelSlot();
        }

        auto modelEdge =
            _modelGraph->connectSlots(dynamic_cast<model::OutputSlot *>(outputSlot),
...
void view::SceneGraph::mouseMoveEvent(QGraphicsSceneMouseEvent *event) {
    ...
    if (_connectionEdge->getDirection() !=
        ConnectionEdge::CreationDirection::UNKNOWN) {

        auto candidateSlot =
            dynamic_cast<Slot *>(itemAt(event->scenePos(), QTransform()));

        if (candidateSlot != nullptr) {
            bool eligibleStatus =
                candidateSlot->acceptConnection(_connectionEdge->getHeadSlot());
            if (eligibleStatus) {
...
void view::SceneGraph::bringToFront(Node *node) const {
    ...
    std::for_each(collidingItemsList.begin(), collidingItemsList.end(),
                  [node](QGraphicsItem *collidingItem) {
...
void view::SceneGraph::duplicateSelectedItems() {
    ...
    QMessageBox::warning(views().first(), "Warning", "Not yet implemented :(");
}
...
void view::SceneGraph::renameSelectedNode() {
    ...
    auto originalNode = selectedNodes[0];
    bool okSelected = false;
    auto newName = QInputDialog::getText(
        views()[0]->parentWidget(), "Rename Node",
        "New name :", QLineEdit::Normal, originalNode->getModelNode()->getName(),
        &okSelected);
...
void view::SceneGraph::rearrangeNodes() const {
    ...
    std::for_each(startingNodes.begin(), startingNodes.end(),
                  [=, &leftMost](model::Node *modelNode) {
    ...
    std::for_each(viewNodesList.begin(), viewNodesList.end(),
                  [leftMost, levelWidth](view::Node *viewNode) {
        auto p = viewNode->scenePos();
        viewNode->setPos(
            leftMost + levelWidth * viewNode->getHorizontalLevel(),
            p.y());
    });
    ...
    while (true) {
        ...
        std::for_each(
            viewNodesList.begin(), viewNodesList.end(), [=](view::Node *viewNode) {
                auto currentCenter = viewNode->getCenter();

                ////////////////////
                std::for_each(
                    viewNodesList.begin(), viewNodesList.end(),
                    [=](view::Node *neighborNode) {
                        ...
                        if (getModelGraph()->checkIsParent(neighborNode->getModelNode(),
                                                           viewNode->getModelNode())) {
                            if (dist < minThreshold) {
                                // move away from neighbor
                                viewNode->setPos(viewNode->scenePos() -
                                                 (dirVec * deltaStep));
                            } else {
                            ...
                            if (dist < maxThreshold) {
                                ...
                                neighborNode->setPos(neighborNode->scenePos() +
                                                     (dirVec * deltaStep));
                                auto neighborChildrenNodes = getNodeChildren(neighborNode);
                                std::for_each(
                                    neighborChildrenNodes.begin(),
                                    neighborChildrenNodes.end(),
                                    [dirVec, deltaStep](view::Node *neighborChild) {
                                        neighborChild->setPos(neighborChild->scenePos() +
                                                              (dirVec * deltaStep));
                                    });
                            }
                        }
...
QRectF view::SceneGraph::computeBoundingRect(const QList<Node *> &nodes,
                                             qreal padding) const {
    if (nodes.isEmpty())
        return QRectF();
    ...
    return QRect(left - padding, top - padding, (right - left) + 2 * padding,
                 (bottom - top) + 2 * padding);
}

void view::SceneGraph::setBackgroundType(
    view::SceneGraph::BackgroundType bgType) {
    _backgroundType = bgType;
}

int view::SceneGraph::getBackgroundType() const { return _backgroundType; }

void view::SceneGraph::setEdgeType(view::SceneGraph::EdgeType type) {
    _edgeType = type;
}

int view::SceneGraph::getEdgeType() const { return _edgeType; }

view::MiniMap *view::SceneGraph::getMiniMap() const { return _miniMap; }
...
QImage view::SceneGraph::takeScreenShot(QRectF rect, qreal multiplier) {
    ...
    QImage img(outputRect.width(), outputRect.height(),
...
void view::SceneGraph::addItem(QGraphicsItem *item) {
    ...
    //nodes.removeAll(node);
            ...
            clearSelection();
            update();
            qreal multiplier = 1.2;
            /*for(auto node : nodes)
        {
            QList<Node*> tempList;
            tempList << node;
            auto rect = computeBoundingRect(tempList, 20);
            //takeScreenShotSvg(QString("./%1-%2-%3.svg").arg(getModelGraph()->getName().replace("
   ", "_"), node->getModelNode()->getType(),
   node->getModelNode()->getName()), rect); QString path =
   QString("./%1-%2-%3.png") .arg(getModelGraph()->getName().replace(" ",
   "_"), node->getModelNode()->getType(),
            ...
            // takeScreenShot(computeBoundingRect(nodes, 20),
            // multiplier).save(QString("./%1.png").arg(getModelGraph()->getName().replace("
            // ", "_")));
            /*
        takeScreenShotSvg(QString("./%1.svg")
                              .arg(getModelGraph()->getName().replace(" ",
   "_")), computeBoundingRect(nodes, 20));
        */
            // takeScreenShotSvg(computeBoundingRect(getNodes(), 20), 3);
            // takeScreenShot().save(QString("./%1.png").arg(getModelGraph()->getName().replace("
            // ", "_"))); emit nodeDoubleClicked(node);
        });
    }
}

...
view::Node::Node(model::Node *modelNode, QString nodeType)
    : model::PersistableEntity(this), _node(modelNode), _nodeType(nodeType) {
...
void view::Node::setupContentWidget() {
  ...
  _contentWidget->setGeometry(_edgeSize, top, _size.width() - 2 * _edgeSize,
                              bottom);
...
void view::Node::setSize(int w, int h) {
  ...
  if (_contentWidget != nullptr) {
    ...
    float tempH =
        std::max(h, (int)headerSizeY + 2 * spacing + minSize.height());
...
bool view::Node::loadFromJSValue(const QJSValue &value) {
  auto positionJsValue =
      model::PersistenceHandler::jsValueToPointF(value.property("position"));
  auto sizeJsValue =
      model::PersistenceHandler::jsValueToPointF(value.property("size"));

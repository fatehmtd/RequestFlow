...
model::ExternalNode::ExternalNode(model::Graph *graph)
    : model::Node(graph, "External") {
    setDeltaDelay(60);
    ...
    if (shouldRunExternal() || !_firstTimeDataSet) {
        ...
        if (_linkedNode != nullptr) {
            ...
            if (_linkedNode->getGraph()->getActiveEnvironment() == nullptr) {
                _linkedNode->getGraph()->setActiveEnvironment(
                    getGraph()->getActiveEnvironment());
            }
            if (externalGraph->start() != Graph::ErrorCode::OK) {
                fail("Error starting the external Scenario");
            }
        } else {
...
void model::ExternalNode::setLinkedNode(model::Node *node) {
    ...
    if (node != nullptr) {
        ...
        _linkedNode = node;
        auto output = _linkedNode->getOutputSlots()[0];
        connect(output, &model::OutputSlot::dataSent, this,
                &ExternalNode::onExternalNodeDataReceived);
...
model::Node *model::ExternalNode::getLinkedNode() const { return _linkedNode; }

void model::ExternalNode::setDeltaDelay(quint64 delay) { _deltaDelay = delay; }

quint64 model::ExternalNode::getDeltaDelay() const { return _deltaDelay; }

void model::ExternalNode::setCustomScript(const QString &script) {
    _customScript = script;
}

QString model::ExternalNode::getCustomScript() const { return _customScript; }
...
void model::ExternalNode::prepareNodeInternals() {
    auto graph = getGraph()->getProject()->findChild<model::Graph *>(
        getExternalGraphId(), Qt::FindChildOption::FindDirectChildrenOnly);
    if (graph != nullptr) {

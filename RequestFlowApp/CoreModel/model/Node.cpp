...
model::Node::Node(Graph* graph, const QString& typeName)
    : NotifiableEntity(graph)
{
    setType(typeName);
    connect(
        graph, &Graph::preparingStartup, this, [this]() {
            clear();
        },
        Qt::ConnectionType::QueuedConnection);

    connect(
        graph, &Graph::started, this, [this]() {
            onGraphStart();
        },
        Qt::ConnectionType::QueuedConnection);

    connect(
        graph, &Graph::stopped, this, [this]() {
            onGraphStop();
        },
        Qt::ConnectionType::QueuedConnection);

    //connect(this, &Node::readyForEvaluation, this, &Node::evaluate);
    connect(this, &Node::evaluated, graph, &Graph::onNodeEvaluated, Qt::ConnectionType::QueuedConnection);
    connect(this, &Node::ready, graph, &Graph::onNodeReady, Qt::ConnectionType::QueuedConnection);
    connect(this, &Node::failed, graph, &Graph::onNodeFailed, Qt::ConnectionType::QueuedConnection);
...
model::OutputSlot* model::Node::addOutputSlot(QString name, int dataType)
{
    ...
    connect(
        this, &Node::evaluated, slot, [slot]() { slot->sendData(); }, Qt::ConnectionType::QueuedConnection);
